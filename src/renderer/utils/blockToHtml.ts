...
function mapBootstrapClassToTailwind(cls: string): string[] {
    ...
    switch (value) {
        ...
        case 'alert':
            return ['p-4', 'mb-4', 'rounded-lg', 'border'];
        case 'alert-primary':
        ...
        case 'alert-dismissible':
            return ['relative', 'pr-10'];
        case 'btn-close':
            return ['absolute', 'top-4', 'right-4', 'text-gray-400', 'hover:text-gray-900', 'bg-transparent', 'border-0'];

        case 'badge':
            return ['inline-block', 'py-1', 'px-2', 'text-xs', 'font-bold', 'leading-none', 'text-center', 'whitespace-nowrap', 'align-baseline', 'rounded'];

        case 'progress':
            return ['flex', 'overflow-hidden', 'h-4', 'text-xs', 'bg-gray-200', 'rounded-full'];
        case 'progress-bar':
            return ['flex', 'flex-col', 'justify-center', 'text-center', 'text-white', 'bg-blue-600', 'transition-all', 'duration-500'];
        ...
        case 'progress-bar-animated':
            return ['animate-[progress-bar-stripes_1s_linear_infinite]'];

        case 'spinner-border':
            return ['inline-block', 'w-8', 'h-8', 'align-[-0.125em]', 'border-4', 'border-current', 'border-r-transparent', 'rounded-full', 'animate-spin'];
        case 'spinner-grow':
            return ['inline-block', 'w-8', 'h-8', 'bg-current', 'rounded-full', 'opacity-0', 'animate-pulse'];
        case 'spinner-border-sm':
            return ['w-4', 'h-4', 'border-2'];
        case 'spinner-grow-sm':
            return ['w-4', 'h-4'];

        case 'form-check':
            return ['flex', 'items-center', 'mb-2'];
        case 'form-check-label':
            return ['inline-block', 'ml-2', 'text-gray-700'];
        ...
        case 'form-range':
            return ['w-full', 'h-2', 'bg-gray-200', 'rounded-lg', 'appearance-none', 'cursor-pointer', 'accent-blue-600'];

        case 'form-control-sm':
            return ['py-1', 'px-2', 'text-sm', 'rounded'];
        case 'form-control-lg':
            return ['py-2', 'px-4', 'text-lg', 'rounded-lg'];

        case 'breadcrumb':
            return ['flex', 'flex-wrap', 'py-2', 'px-4', 'mb-4', 'list-none', 'bg-gray-50', 'rounded-md'];
        case 'breadcrumb-item':
            return ['inline-block', 'mr-2'];
        case 'active':
            return ['text-gray-500'];

        case 'pagination':
            return ['flex', 'list-none', 'rounded', 'items-center', 'flex-wrap'];
        case 'page-item':
            return ['relative', 'block'];
        case 'page-link':
            return ['relative', 'block', 'py-1.5', 'px-3', 'bg-white', 'border', 'border-gray-300', 'rounded', 'text-gray-800', 'hover:text-gray-900', 'hover:bg-gray-100'];
        case 'pagination-sm':
            return ['text-sm'];
        case 'pagination-lg':
            return ['text-lg'];
        ...
        case 'table':
            return ['w-full', 'border-collapse', 'text-left', 'text-sm'];
        case 'table-striped':
            return ['[&>tbody>tr:nth-child(odd)]:bg-[rgba(0,0,0,0.03)]'];
        case 'table-bordered':
            return ['border', 'border-[var(--theme-border)]', '[&>thead>tr>th]:border', '[&>tbody>tr>td]:border', '[&>thead>tr>th]:border-[var(--theme-border)]', '[&>tbody>tr>td]:border-[var(--theme-border)]'];
        case 'table-hover':
            return ['[&>tbody>tr:hover]:bg-[rgba(0,0,0,0.05)]'];
        case 'table-sm':
            return ['text-xs'];
        case 'table-dark':
        ...
        case 'dropdown-toggle-split':
            return ['px-3'];
        case 'dropdown-menu':
            return ['absolute', 'z-20', 'mt-2', 'min-w-[12rem]', 'rounded-md', 'border', 'border-[var(--theme-border)]', 'bg-[var(--theme-surface)]', 'p-1', 'shadow-lg'];
        case 'dropdown-menu-end':
            return ['right-0'];
        case 'dropdown-menu-dark':
            return ['bg-slate-900', 'text-white', 'border-slate-700'];
        case 'dropdown-item':
            return ['block', 'w-full', 'rounded-sm', 'px-3', 'py-2', 'text-sm', 'text-[var(--theme-text)]', 'no-underline', 'hover:bg-[rgba(0,0,0,0.06)]'];
        case 'dropdown-divider':
            return ['my-1', 'border-t', 'border-[var(--theme-border)]'];

        case 'offcanvas':
            return ['fixed', 'z-50', 'bg-[var(--theme-surface)]', 'shadow-xl', 'border', 'border-[var(--theme-border)]', 'transition-transform'];
        case 'offcanvas-start':
            return ['left-0', 'top-0', 'h-full', 'w-80'];
        case 'offcanvas-end':
            return ['right-0', 'top-0', 'h-full', 'w-80'];
        case 'offcanvas-top':
            return ['left-0', 'top-0', 'h-64', 'w-full'];
        case 'offcanvas-bottom':
            return ['left-0', 'bottom-0', 'h-64', 'w-full'];
        case 'offcanvas-header':
            return ['flex', 'items-center', 'justify-between', 'border-b', 'border-[var(--theme-border)]', 'px-4', 'py-3'];
        case 'offcanvas-title':
            return ['text-lg', 'font-semibold'];
        case 'offcanvas-body':
            return ['p-4', 'overflow-y-auto'];
        case 'stats-section':
            return ['py-12'];
        case 'stats-item':
            return ['h-full', 'rounded-lg', 'p-6'];
        case 'team-grid':
            return ['py-12'];
        case 'team-member-card':
            return ['h-full', 'rounded-xl', 'border', 'border-[var(--theme-border)]', 'bg-[var(--theme-surface)]', 'p-4', 'shadow-sm'];
        case 'gallery':
            return ['py-12'];
        case 'gallery-item':
        ...
        case 'process-steps':
            return ['py-12'];
        case 'process-step':
            return ['relative', 'rounded-lg', 'p-4'];
...
function getBlockContent(
    ...
    switch (type) {
        ...
        <div class="${i === defaultTab ? '' : 'hidden '}rounded-xl border border-[var(--theme-border)] bg-[var(--theme-surface)] p-4 text-[var(--theme-text)]" id="${id}-content-${i}" data-tw-tab-panel>
          ${tab.blocks && tab.blocks.length > 0 ? blockToHtml(tab.blocks, {
                    framework,
                    includeDataAttributes: !isExportMode,
...
${pad}</button>`
    ...
    if (block.type === 'countdown') {
        ...
        const labels = block.props.labels as any || {
            days: 'Days',
            hours: 'Hours',
            minutes: 'Minutes',
            seconds: 'Seconds'
...
export function pageToHtml(blocks: Block[], options: PageHtmlOptions = {}): string {
    ...
    const bodyHtml = blockToHtml(blocks, {
        ...blockOptions,
        indent: 1,
        pages: options.pages,
