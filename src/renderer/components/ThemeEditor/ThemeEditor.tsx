...
function SpacingTab({
  ...
  return (
    <div className="theme-section">
      ...
      <div className="theme-section-title" style={{ marginTop: 16 }}>
        Scale Preview
      </div>
        ...
        <div className="theme-field">
          ...
          <div className="theme-color-input-row">
            <div
              ...
              style={{ backgroundColor: borders.color }}
            >
              <input
                type="color"
                value={borders.color}
    ...
    <div className="theme-section">
      <div className="theme-section-header">
        <div className="theme-section-title">
          Theme Presets for{" "}
          {editingMode === "light" ? "Light Page" : "Dark Page"}
        </div>
        <button
              ...
              return (
                ...
                >
                  ...
                  <div className="theme-preset-header">
                    <div className="theme-preset-heading-group">
                      ...
                      <div className="theme-preset-meta-row">
                        <span className="theme-preset-category-pill">
                          {preset.category}
                        </span>
                      </div>
                    </div>
                  ...
                  <div className="theme-preset-tags">
                    {preset.tags.map((tag) => (
                      <span
                        key={`${preset.theme.name}-${tag}`}
                        className="theme-preset-tag"
                      >
                        {tag}
                      </span>
                    ))}
                  ...
                  <div className="theme-preset-swatches">
                    ...
                    <div
                      className="theme-preset-swatch"
                      style={{ backgroundColor: preset.theme.colors.text }}
                    />
                  </div>
                </div>
              );
      ...
      {customPresets.length > 0 && (
        <div
          ...
          style={{ marginTop: filteredBuiltInThemes.length > 0 ? 20 : 0 }}
        >
          Custom Presets
        </div>
      )}
          ...
          return (
            ...
            >
              ...
              <div className="theme-preset-header">
                {isEditing ? (
                  <div
                    className="theme-preset-edit-form"
                  ...
                  <>
                    <div className="theme-preset-heading-group">
                      <div className="theme-preset-name">
                        {preset.name}
                        {isCustom && (
                          <span className="theme-preset-badge">Custom</span>
                        )}
                      </div>
                    </div>
                    {isCustom && (
...
export default function ThemeEditor({
  ...
  const handleReset = useCallback(() => {
    const resetTheme =
      editingMode === "dark" ? createDefaultDarkTheme() : createDefaultTheme();
    setProjectTheme(resetTheme, editingMode);
    showToast(
      `${editingMode === "dark" ? "Dark" : "Light"} page theme reset to defaults`,
      "success",
    );
    ...
    input.onchange = async () => {
      ...
      try {
        ...
        const imported: ProjectTheme = {
          ...
          customCss:
            typeof parsed.customCss === "string" ? parsed.customCss : "",
          customCssFiles: Array.isArray(parsed.customCssFiles)
            ? parsed.customCssFiles
            : [],
        };
        ...
        showToast(
          `Theme "${imported.name}" imported and applied to ${editingMode} page`,
          "success",
        );
      } catch (err) {
  ...
  const handleUpdatePreset = useCallback(
    (name: string, preset: ProjectTheme) => {
  ...
  const galleryThemesForMode = builtInGalleryThemes.filter(
        ...
        <div className="theme-editor-content">
          ...
          {activeTab === "fonts" && (
            <FontManager
              ...
              onTypographyChange={(patch: Partial<ThemeTypography>) =>
                updateThemeTypography(patch, editingMode)
              }
            />
          )}
        </div>
