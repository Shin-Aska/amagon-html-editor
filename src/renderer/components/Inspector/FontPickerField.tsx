...
export default function FontPickerField({
  ...
  const openDropdown = () => {
    ...
    setPortalStyle({
      position: "fixed",
      top: flipUp
        ? rect.top - Math.min(dropH, spaceAbove - 12)
        : rect.bottom + 3,
      left: rect.left,
      width: Math.max(rect.width, 220),
      maxHeight: flipUp
        ? Math.min(dropH, spaceAbove - 12)
        : Math.min(dropH, spaceBelow - 12),
      zIndex: 99999,
    });
    setSearch("");
    setOpen(true);
  ...
  const dropdown = (
    <div
      ...
      style={portalStyle as React.CSSProperties}
    >
      ...
      <div className="fp-list" ref={listRef}>
        {groups.length === 0 && (
          <div className="fp-empty">No fonts match "{search}"</div>
        )}
