...
describe('projectStore', () => {
  ...
  describe('project settings', () => {
    it('updates project name', () => {
      ...
      expect(store.settings.name).toBe('My Project')
    })
    ...
    it('updates framework', () => {
      ...
      expect(store.settings.framework).toBe('vanilla')
    })

    it('merges partial settings', () => {
      store.updateSettings({ name: 'New Name' })
      expect(store.settings.framework).toBe('bootstrap-5')
    })
  })
  ...
  describe('pages', () => {
    it('adds a new page', () => {
      ...
      expect(store.pages).toHaveLength(2) // Default page + new page
    ...
    it('generates unique slugs for duplicate titles', () => {
      ...
      expect(store.pages[1].slug).toBe('about')
      expect(store.pages[2].slug).toBe('about-1')
    })
    ...
    it('removes a page', () => {
      ...
      expect(store.pages.find((p) => p.id === page.id)).toBeUndefined()
    })
    ...
    it('prevents removing the last page', () => {
      ...
      expect(store.pages[0].title).toBe('Home')
    })
    ...
    it('updates page blocks', () => {
      ...
      expect(store.pages[0].blocks[0].type).toBe('heading')
    })
    ...
    it('updates page meta', () => {
      ...
      expect(store.pages[0].meta.title).toBe('Custom Title')
      expect(store.pages[0].meta.description).toBe('Desc')
    })

    it('gets current page', () => {
      const current = store.getCurrentPage()
      expect(current).toBeDefined()
      expect(current?.id).toBe(store.currentPageId)
    })
  })
  ...
  describe('user blocks', () => {
    it('adds a user block', () => {
      ...
      expect(store.userBlocks).toHaveLength(1)
      expect(store.userBlocks[0].label).toBe('My Component')
    })
    ...
    it('removes a user block', () => {
      ...
      expect(store.userBlocks).toHaveLength(0)
    })
  })
  ...
  describe('project data import/export', () => {
    ...
    it('imports project data', () => {
      ...
      expect(store.settings.name).toBe('Imported')
      expect(store.pages).toHaveLength(1)
      expect(store.currentPageId).toBe('p-imported')
      expect(store.filePath).toBe('/path/to/project.hoarses')
    })
  ...
  describe('file path management', () => {
    it('sets file path', () => {
      ...
      expect(store.filePath).toBe('/home/user/project.hoarses')
    })

    it('marks project as loaded after import', () => {
      expect(store.isProjectLoaded).toBe(false)
      ...
      expect(store.isProjectLoaded).toBe(true)
    })
  })
})
