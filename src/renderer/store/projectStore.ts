...
interface ProjectActions {
  ...
  updateProjectTheme: (
    patch: Partial<ProjectTheme>,
    mode?: PageThemeMode,
  ) => void;
  updateThemeColors: (
    patch: Partial<ProjectTheme["colors"]>,
    mode?: PageThemeMode,
  ) => void;
  updateThemeTypography: (
    patch: Partial<ProjectTheme["typography"]>,
    mode?: PageThemeMode,
  ) => void;
  updateThemeSpacing: (
    patch: Partial<ProjectTheme["spacing"]>,
    mode?: PageThemeMode,
  ) => void;
  updateThemeBorders: (
    patch: Partial<ProjectTheme["borders"]>,
    mode?: PageThemeMode,
  ) => void;
...
function removeMetaKeysFromCounts(
  counts: Record<string, number>,
  meta: Record<string, string> | undefined,
): void {
  if (!meta) return;
...
function isProjectTheme(value: unknown): value is ProjectTheme {
  if (!value || typeof value !== "object") return false;
  const candidate = value as Record<string, unknown>;
  return typeof candidate.name === "string" && isThemeColors(candidate.colors);
}

function getThemePackTheme(
  pack: ThemePack,
  mode: PageThemeMode,
): ProjectTheme | null {
...
export const useProjectStore = create<ProjectStore>((set, get) => {
  ...
  return {
    ...
    setProject: (data, filePath) => {
      ...
      const fallbackTheme =
        incoming?.theme &&
        typeof incoming.theme === "object" &&
        (incoming.theme as ProjectTheme).colors
      ...
      const sharedCssFiles = (() => {
        ...
        if (
          ...
          lightTheme.customCssFiles.length > 0
        ) {
      ...
      if (themes) {
        themes.light.customCssFiles = sharedCssFiles.map((file) => ({
          ...file,
        }));
        themes.dark.customCssFiles = sharedCssFiles.map((file) => ({
          ...file,
        }));
      ...
      const nextPages =
        data.pages.length > 0 ? data.pages : [createDefaultPage()];
    ...
    setProjectTheme: (theme, mode = "light") => {
      set((state) => {
        const variants = normalizeThemeVariants(
        ...
        return {
          settings: syncLegacyTheme({
            ...state.settings,
            theme:
              mode === "light" ? cloneTheme(nextTheme) : state.settings.theme,
            themes: nextVariants,
          }),
        };
      });
    ...
    removeThemePack: (id) => {
      set((state) => ({
        ...
        appliedThemePackId:
          state.appliedThemePackId === id ? null : state.appliedThemePackId,
      }));
      markProjectDirty();
    },
    ...
    removeSectionTemplate: (id) => {
      set((state) => ({
        sectionTemplates: state.sectionTemplates.filter(
      ...
      set((state) => ({
        settings: {
          ...
          theme: {
            ...state.settings.theme,
            customCssFiles: [
              ...(state.settings.theme.customCssFiles || []),
              file,
            ],
          },
          themes: {
    ...
    removeCssFile: (id) => {
      set((state) => ({
        settings: {
          ...
          theme: {
            ...state.settings.theme,
            customCssFiles: (state.settings.theme.customCssFiles || []).filter(
              (f) => f.id !== id,
            ),
          },
          themes: {
    ...
    updateCssFile: (id, patch) => {
      set((state) => ({
        settings: {
          ...
          theme: {
            ...state.settings.theme,
            customCssFiles: (state.settings.theme.customCssFiles || []).map(
              (f) => (f.id === id ? { ...f, ...patch } : f),
            ),
          },
          themes: {
        ...
        return {
          settings: {
            ...
            themes: {
              ...
              light: {
                ...variants.light,
                customCssFiles: reorder(variants.light.customCssFiles || []),
              },
              dark: {
                ...variants.dark,
                customCssFiles: reorder(variants.dark.customCssFiles || []),
              },
            },
          },
        };
      ...
      set((state) => ({
        settings: {
          ...
          theme: {
            ...state.settings.theme,
            customCssFiles: (state.settings.theme.customCssFiles || []).map(
              (f) => (f.id === id ? { ...f, enabled: !f.enabled } : f),
            ),
          },
          themes: {
            ...normalizeThemeVariants(
            ...
            light: {
              ...
              ).map((f) => (f.id === id ? { ...f, enabled: !f.enabled } : f)),
            },
            dark: {
              ...normalizeThemeVariants(
                state.settings.themes,
    ...
    removePage: (id) => {
      set((state) => {
        ...
        const newCurrentId =
          state.currentPageId === id ? filtered[0].id : state.currentPageId;
